...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        os.makedirs(f"./logs/{args.model_name}/{args.dataset_name}/{args.save_model_name}//{args.anomaly_ratio}/{args.negative_sample_strategy}", exist_ok=True)
        # create file handler that logs debug and higher level messages
        fh = logging.FileHandler(f"./logs/{args.model_name}/{args.dataset_name}/{args.save_model_name}/{args.anomaly_ratio}/{args.negative_sample_strategy}/{str(time.time())}.log")
        ...
        save_model_folder = f"./saved_models/{args.model_name}/{args.dataset_name}/{args.save_model_name}/{args.anomaly_ratio}/{args.negative_sample_strategy}/"
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.dataset_name not in ['BGL', 'reddit', 'wikipedia']:
            ...
            # labels = torch.cat([torch.from_numpy(batch_labels).float().to(predicts.device), torch.ones_like(negative_probabilities)], dim=0)
            ...
            if (epoch + 1) % args.test_interval_epochs == 0:
                ...
                for metric_name, metric_value in new_node_test_metrics.items():
                    logger.info(f'new node test {metric_name}, {metric_value:.4f}')
        ...
        save_result_folder = f"./saved_results/{args.model_name}/{args.dataset_name}/{args.anomaly_ratio}/{args.negative_sample_strategy}/"
