...
def generate_anomalies(model, num_samples=1000):
    model.eval()
    z = torch.randn(num_samples, latent_dim).to(device)  
    synthetic_features = model.inverse(z)
    log_likelihoods = model.log_prob(synthetic_features)
    anomalies = synthetic_features[log_likelihoods < threshold]
    return anomalies

def convert_to_anomaly_scores(logps):
     # Normalize log-likelihoods to (-Inf, 0]
    logps = logps - torch.max(logps) # Subtract max to avoid overflow
    # Convert to probabilities in range [0, 1]
    scores = 1 - torch.exp(logps)
    return scores
