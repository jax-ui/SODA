...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    graph_df = pd.read_csv('/{}/ml_{}.csv'.format(dataset_name, dataset_name))
    edge_raw_features = np.load('/{}/ml_{}.npy'.format(dataset_name, dataset_name))
    node_raw_features = np.load('/{}/ml_{}_node.npy'.format(dataset_name, dataset_name))
    ...
    mask_norm = train_data.labels == 0
    mask_abnorm = train_data.labels == 1

    train_norm_data = Data(
        src_node_ids=train_data.src_node_ids[mask_norm],
        dst_node_ids=train_data.dst_node_ids[mask_norm],
        node_interact_times=train_data.node_interact_times[mask_norm],
        edge_ids=train_data.edge_ids[mask_norm],
        labels=train_data.labels[mask_norm]
    )

    train_abnorm_data = Data(
        src_node_ids=train_data.src_node_ids[mask_abnorm],
        dst_node_ids=train_data.dst_node_ids[mask_abnorm],
        node_interact_times=train_data.node_interact_times[mask_abnorm],
        edge_ids=train_data.edge_ids[mask_abnorm],
        labels=train_data.labels[mask_abnorm]
    )
    ...
    return node_raw_features, edge_raw_features, full_data, train_data, train_norm_data, train_abnorm_data, mask_norm, mask_abnorm, val_data, test_data, new_node_val_data, new_node_test_data
