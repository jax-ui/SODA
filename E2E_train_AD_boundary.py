...
from utils.losses import get_logp_boundary, calculate_bg_spp_loss,normal_fl_weighting, abnormal_fl_weighting
from models.modules import MergeLayer, flow_model, subnet_fc
from utils.utils import set_random_seed, convert_to_gpu, get_parameter_sizes, create_optimizer, get_logp, convert_to_anomaly_scores
...
from Im_losses import LDAMLoss, FocalLoss, VSLoss
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        AD_layer = flow_model(2*node_raw_features.shape[1])
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                # AD_loss = task_loss(anomaly_score, labels)
                ...
                if epoch > 5:
                    ...
                    bound_loss = loss_n_con + loss_a_con
                
                    loss = ml_loss + bound_loss
                    # loss = AD_loss + 0.1 * bound_loss
                else:
                    # if args.focal_weighting:
                    #     normal_weights = normal_fl_weighting(logps.detach())
                    #     ml_loss = -log_sigmoid(logps) * normal_weights
                    #     ml_loss = ml_loss.mean()
                    #     loss =  ml_loss
                    # else:
                    ml_loss = -log_sigmoid(logps[batch_labels == 0]).mean()
                    loss = ml_loss
