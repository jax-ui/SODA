...
class GraphMixer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, num_tokens: int, num_layers: int = 2, token_dim_expansion_factor: float = 1,
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_node_embeddings = self.compute_node_temporal_embeddings(node_ids=src_node_ids, node_interact_times=node_interact_times, edge_ids= edge_ids,
                                                                    num_neighbors=num_neighbors, time_gap=time_gap)
        # Tensor, shape (batch_size, node_feat_dim)
        dst_node_embeddings = self.compute_node_temporal_embeddings(node_ids=dst_node_ids, node_interact_times=node_interact_times, edge_ids= edge_ids,
                                                                    num_neighbors=num_neighbors, time_gap=time_gap)
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray, edge_ids: np.ndarray,
        ...
        all_node_ids = torch.cat([torch.from_numpy(neighbor_node_ids),torch.from_numpy(node_ids).unsqueeze(dim=-1)], dim = 1)
        # Tensor, shape (batch_size, 1, edge_feat_dim)
        edge_raw_features = self.edge_raw_features[torch.from_numpy(edge_ids)]
        # Tensor, shape (batch_size, num_neighbors, edge_feat_dim)
        nodes_neighbor_edge_raw_features = self.edge_raw_features[torch.from_numpy(neighbor_edge_ids)]
         # Tensor, shape (batch_size, num_neighbors+1, edge_feat_dim)
        nodes_edge_raw_features = torch.cat([nodes_neighbor_edge_raw_features,edge_raw_features.unsqueeze(dim=1)], dim = 1)
        ...
        times = torch.cat([torch.from_numpy(neighbor_times),torch.from_numpy(node_interact_times).unsqueeze(dim=-1)],dim = 1)
        nodes_neighbor_time_features = self.time_encoder(timestamps=times.float().to(self.device))
        
        # ndarray, set the time features to all zeros for the padded timestamp
        nodes_neighbor_time_features[all_node_ids == 0] = 0.0
        
        # Tensor, shape (batch_size, num_neighbors+1, edge_feat_dim + time_feat_dim)
        combined_features = torch.cat([nodes_edge_raw_features, nodes_neighbor_time_features], dim=-1)
        # edge_time_features = self.time_encoder(timestamps=torch.from_numpy(node_interact_times).unsqueeze(dim=-1).float().to(self.device))
        # combined_features = torch.cat([edge_raw_features.unsqueeze(dim=1), edge_time_features], dim=-1)
        
        # Tensor, shape (batch_size, num_neighbors+1, num_channels)
        ...
        output_node_features = self.node_raw_features[torch.from_numpy(node_ids)]
        # Tensor, shape (batch_size, node_feat_dim)
