...
class TCL(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                                                 node_interact_times: np.ndarray, edge_ids: np.ndarray, num_neighbors: int = 20):
        """
        ...
        # # ndarray, shape (batch_size, num_neighbors + 1)
        # src_neighbor_node_ids = np.concatenate((src_neighbor_node_ids, src_node_ids[:, np.newaxis]), axis=1)
        # src_neighbor_edge_ids = np.concatenate((src_neighbor_edge_ids, edge_ids[:, np.newaxis]), axis=1)
        ...
        # dst_neighbor_edge_ids = np.concatenate((dst_neighbor_edge_ids, edge_ids[:, np.newaxis]), axis=1)
        # dst_neighbor_times = np.concatenate((dst_neighbor_times, node_interact_times[:, np.newaxis]), axis=1)

        # #only edge itself:
        src_neighbor_node_ids = src_node_ids[:, np.newaxis]
        src_neighbor_edge_ids = edge_ids[:, np.newaxis]
        src_neighbor_times =node_interact_times[:, np.newaxis]

        # ndarray, shape (batch_size, num_neighbors + 1)
        dst_neighbor_node_ids = dst_node_ids[:, np.newaxis]
        dst_neighbor_edge_ids = edge_ids[:, np.newaxis]
        dst_neighbor_times = node_interact_times[:, np.newaxis]
