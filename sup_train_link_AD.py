...
from Im_losses import FocalLoss, LDAMLoss
...
if __name__ == "__main__":
    ...
    test_neg_edge_sampler = NegativeEdgeSampler(src_node_ids=full_data.src_node_ids, dst_node_ids=full_data.dst_node_ids, seed=1)
    ...
    cls_num_list = [np.sum(labels == 0), np.sum(labels == 1)]
    ...
    for run in range(args.num_runs):
        ...
        os.makedirs(f"./logs/sup/{args.loss_type}/{args.model_name}/{args.dataset_name}/{args.save_model_name}", exist_ok=True)
        # create file handler that logs debug and higher level messages
        fh = logging.FileHandler(f"./logs/sup/{args.loss_type}/{args.model_name}/{args.dataset_name}/{args.save_model_name}/{str(time.time())}.log")
        ...
        model = nn.Sequential(dynamic_backbone, AD_layer)
        ...
        save_model_folder = f"./saved_models/sup/{args.model_name}/{args.dataset_name}/{args.save_model_name}/"
        ...
        for epoch in range(args.num_epochs):
            if args.train_rule == 'None':
                ...
                per_cls_weights = per_cls_weights / np.sum(per_cls_weights) * len(cls_num_list)
                per_cls_weights = torch.FloatTensor(per_cls_weights).cuda(args.gpu)
                per_cls_weights = per_cls_weights * scale
            ...
            if args.loss_type == 'CE':
                task_loss = nn.CrossEntropyLoss(weight=per_cls_weights).cuda(args.gpu)
            elif args.loss_type == 'LDAM':
                task_loss = LDAMLoss(cls_num_list=cls_num_list, max_m=0.5, s=30, weight=per_cls_weights, gpu=args.gpu)
            elif args.loss_type == 'Focal':
                task_loss = FocalLoss(weight=per_cls_weights, gamma=1).cuda(args.gpu)
            elif args.loss_type == 'VS':
                task_loss = VSLoss(cls_num_list=args.cls_num_list, tau=args.tau, gamma=args.gamma,
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                positive_probabilities = model[1](edge_embeddings).squeeze(dim=-1).sigmoid()
                predicts= positive_probabilities
                
                labels = torch.from_numpy(batch_labels).float().to(predicts.device)
                loss = task_loss(predicts, labels)
        ...
        save_result_folder = f"./saved_results/sup/{args.loss_type}/{args.model_name}/{args.dataset_name}/"
